...
function UploadField({ label, kind, onUploaded }: { label: string; kind: "image" | "cv"; onUploaded: (url: string) => void }) {
  ...
  async function upload(file: File | undefined) {
    ...
    if (!response.ok) {
      setState(typeof result.error === "string" ? result.error : "Upload failed.");
      return;
    }
    onUploaded(result.url);
    setState("Uploaded");
  }
  return <label className="admin-upload"><span>{label}</span><input type="file" accept={kind === "cv" ? ".pdf,application/pdf" : "image/jpeg,image/png,image/webp,image/gif"} onChange={(event) => upload(event.target.files?.[0])} /><small>{state || (kind === "cv" ? "PDF up to 10 MB" : "JPG, PNG, WebP, or GIF up to 5 MB")}</small></label>;
}
      ...
      <section className="admin-editor">
        ...
        {activeTab === "advanced" ? <div className="admin-advanced"><div className="admin-code-wrap"><label htmlFor="content-json">Full portfolio content JSON</label><textarea id="content-json" className="admin-code-editor" value={contentLoading ? "Loading content..." : contentJson} onChange={(event) => setContentJson(event.target.value)} spellCheck={false} disabled={contentLoading} /></div><button className="button button-secondary" onClick={parseAdvanced}>Apply JSON draft</button></div> : (
          <div className="admin-visual">
            ...
            <section className="admin-panel-card"><p className="eyebrow">Projects</p><h2>{projectCount} project cards</h2><div className="admin-repeat-grid">{content.projects.map((project, index) => <article className="admin-repeat-card admin-project-editor" key={project.id}><ImagePreview url={project.image} label={`${project.title} preview`} /><UploadField label="Upload project image" kind="image" onUploaded={(value) => updateArrayItem("projects", index, "image", value)} /><Field label="Title" value={project.title} onChange={(value) => updateArrayItem("projects", index, "title", value)} /><Field label="Category" value={project.category} onChange={(value) => updateArrayItem("projects", index, "category", value)} /><Field label="Image URL" value={project.image} onChange={(value) => updateArrayItem("projects", index, "image", value)} /><Field label="Description" value={project.description} onChange={(value) => updateArrayItem("projects", index, "description", value)} multiline /><Field label="Outcome" value={project.outcome} onChange={(value) => updateArrayItem("projects", index, "outcome", value)} multiline /><Field label="Full details" value={project.details} onChange={(value) => updateArrayItem("projects", index, "details", value)} multiline />{project.tools.map((tool, toolIndex) => <Field key={`${project.id}-tool-${toolIndex}`} label={`Tool ${toolIndex + 1}`} value={tool} onChange={(value) => updateListItem("projects", index, "tools", toolIndex, value)} />)}</article>)}</div></section>
